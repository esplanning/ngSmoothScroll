...
(function () {
	...
	var smoothScroll = function (element, options) {
		...
		//Find container by ID or class if options specify
		var container = document.getElementById(options.containerId) || null;
		if(!container && options.containerClass) {
			container = findAncestor(element, options.containerClass);
		}
		...
		setTimeout( function() {
			...
			var stopAnimation = function () {
				...
				if (
					...
					( // condition 3
						internalHeight >= scrollHeight + offset
					)
				) { // stop
